...
cp $SCRIPT_DIR/assist-dirty.yaml $SCRIPT_DIR/assist-dirty-vspec-all.yaml
echo ""                               >> $SCRIPT_DIR/assist-dirty-vspec-all.yaml
echo "contract_execution_clause:"     >> $SCRIPT_DIR/assist-dirty-vspec-all.yaml
echo "  -vspec-all-memory-assists"    >> $SCRIPT_DIR/assist-dirty-vspec-all.yaml

cp $SCRIPT_DIR/assist-accessed.yaml $SCRIPT_DIR/assist-dirty-vspec-all.yaml
...
python $revizor_src/revizor.py fuzz -s $instructions -c  $SCRIPT_DIR/assist-dirty-vspec-all.yaml -i 100 -n 100000000 --timeout $TIMEOUT  -w $SCRIPT_DIR/results/violations/ &> $logfile